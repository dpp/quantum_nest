...
trait Quantum {
  ...
  /** Gets the main channel to send messages to this Quantum. If the `Channel`
    * is closed or the Quantum is finished, the `Box` will be `Empty`
    *
    * @return
  ...
  /** Is the Quantum still executing messages or otherwise waiting for input
    *
    * @return
    */
  ...
  /** Get the typed state
    ...
    *   the manifest
    * @return
    *   a `Full` `Box` if the named state exists and is the correct type
    */
  def getState[T](name: String)(implicit
      manifest: TypeTag[T]
  ): Box[ReadonlyState[T]]
...
object PredicateSource {
  def fromJObject(obj: JsonAST.JField): Vector[Box[PredicateSource]] = {
    obj match {
      case JsonAST.JField("from_message", info) =>
        predicateInfo(info).map {
          case Full((name, varName)) => Full(FromMessageSource(name, varName));
          case Empty                 => Empty; case f: Failure => f
        }
      case JsonAST.JField("from_state", info) =>
        predicateInfo(info).map {
          case Full((name, varName)) => Full(FromStateSource(name, varName));
          case Empty                 => Empty; case f: Failure => f
        }
      case _ => Vector()
    }
    ...
    obj match {
      ...
      case jo: JObject => {
        (jo \ "from", jo \ "var") match {
          case (JString(from), JString(variable)) =>
            Vector(Full((from, Full(variable.trim()))))
          case (JString(from), bad) =>
            Vector(
              Full(
                (
                  from,
                  Failure(f"Could not convert ${bad} into a variable name")
                )
              )
            )
          case bad =>
            Vector(Failure(f"Could not get `from` and `var` from ${bad}"))
        }
      }
      case other =>
        Vector(
          Failure(f"Could not get predicate `from` and `var` from ${other}")
        )
    }
  }
}
...
final case class FromMessageSource(message: String, varName: Box[String])
  ...
  def updateVarName(newVarName: String): PredicateSource =
    FromMessageSource(message, Full(newVarName))
}
final case class FromStateSource(state: String, varName: Box[String])
    extends PredicateSource {
  def updateVarName(newVarName: String): PredicateSource =
    FromStateSource(state, Full(newVarName))
}

case class StateDef(
    name: String,
    predicates: Seq[PredicateSource],
...
object StateDef {
  def from(namespace: String, definition: JsonAST.JField): Box[StateDef] = {
    ...
    val allFields = definition.value match {
      case JObject(fields) => Full(fields)
      case JArray(arr) =>
        Full(arr.flatMap {
      ...
      case v =>
        Failure(
          f"Cannot define state ${name} because the definition must be an Object or Array, instead got ${v}"
        )
    }
    allFields match {
      case Full(fields) => {
        val predicatesBase =
          fields.flatMap(f => PredicateSource.fromJObject(f)).toVector

        val failedPredicates = predicatesBase.flatMap {
          case f: Failure => List(f); case _ => Nil
        }
        ...
        // build the execs
        val execStuff = fields
          .map(f => ExecFunction.fromJObject(namespace, f, varNames))
          .toVector

        val execFailures = execStuff
          .flatMap {
        ...
        if (predicates.length == 0) {
          ...
          val returnList = Failure(
            f"Unable to compile State information for ${name}"
          ) :: execFailures.toList
          ...
          def chainFailures(ff: List[Failure]): Failure = {
            ff match {
              ...
              case v :: Nil => v
              case v :: rest =>
                Failure(v.msg, v.exception, Full(chainFailures(rest)))
            }
          }
          ...
          Failure(
            f"For State ${name}, there are ${predicates.length} but no function to convert them into a value"
          )
        } else {
...
class SimpleQuantum(val definition: QuantumDefinition, val name: String)
    extends Quantum {
  ...
  def SimpleQuantum(defined: QuantumDefinition, n: String): Unit = {
  ...
  def getState[T](
      name: String
  )(implicit manifest: TypeTag[T]): Box[ReadonlyState[T]] = {
    states.get().get(name) match {
      case Some(state) if state.manifest == manifest =>
        Full(state).asInstanceOf[Box[ReadonlyState[T]]]
      case _ => Empty
    }
...
trait ReadonlyState[T] {
  ...
  /** The value of the state. `Empty` if the predicates of the state have not
    * yet been resolved
    *
    * @return
    *   a `Box` containing the value of the state
    */
  def value: Box[T]

  /** Has the State been resolved (all its predicates have been satisfied and
  ...
  /** What Quantum owns this State?
    *
    * @return
    *   a references to the owning Quantum
    */
  def owner: Quantum
