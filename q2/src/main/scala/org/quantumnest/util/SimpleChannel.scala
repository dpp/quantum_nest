...
class SimpleChannel[T](implicit manifest: TypeTag[T]) extends Channel[T] {
  ...
  /** Each channel has a unique UUID. Comparing two channels is the comparison
    * of the UUID and nothing else
    *
    * @param x
    *   the other thing to compare
    * @return
    *   true if the thing is a Channel and it has the same UUID as this channel
    */
...
trait SendChannel[T] {
  ...
  /** Get a UUID for this channel. The UUID can be used within the same
    * Classloader to look up a channel
    *
    * @return
  ...
  /** Get the URL of the Channel. The URL can be used anywhere (across a
    * cluster) to locate a channel, even if the locus of the channel has
    * migrated (e.g. a node goes down, the Quantum processing the channel has
    * been brought up on another node)
    *
    * @return
  ...
  /** Is the channel closed? This value may not be accurate as a remote channel
    * may be closed, but a `SendChannel` proxy may not be updated.
    *
    * @return
    *   true of the channel is no longer processing messages
    */
  def closed_? : Boolean
}
...
trait ReceiveChannel[T] {
  ...
  /** Receive a message or timeout trying. If the `Channel` is closed, an
...
object Channel {
  ...
  def apply[T]()(implicit manifest: TypeTag[T]): Channel[T] =
    new SimpleChannel[T]

  def addChannel(channel: Channel[_]): Unit = {
